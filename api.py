...
def get_previous_days_data(): # http://127.0.0.1:5000/pastData?code=AXISBANK&days=15
    ...
    if 'code' in request.args and 'days' in request.args:
        ...
        previous_days_data = web.DataReader(code+'.NS', data_source='yahoo', start=datetime.strftime(start_date, '%Y-%m-%d'), end=datetime.strftime(end_date, '%Y-%m-%d')).round(2)
        previous_days_data.reset_index(level=0, inplace=True)
        previous_days_data = json.loads(previous_days_data.to_json(orient="records"))
